...
- **`void ttyMode(int isRaw)`**
    ...
    - raw にすると、行バッファやエコー等を無効化して **1文字単位**で読めるようになる（ゲームやREPLの即時キー入力向け）。
        
    - REPL開始時に raw にして、終了時に元に戻す…のような用途が典型。
        
- **`int qKey()`**
    
    - “**キーが来ているか？**”を **ブロックせず**に調べる関数（query key）。
...
- **`void ms(cell sleepForMS)`**
    ...
    - 実装は `usleep`/`nanosleep`/Windowsなら`Sleep`など。
...
`strcpy(tib, "argX"); for (int i=0; (i<argc) && (i<10); i++) {     tib[3] = '0' + i;     addLit(tib, (cell)argv[i]); }`
...
#define PRIMS(X) \ X(DUP, "dup", t=TOS; push(t); ) \ X(SWAP, "swap", t=TOS; TOS=NOS; NOS=t; ) \ X(DROP, "drop", pop(); ) \ について再度教えてください。 Cで記述されたWORDのすべてが「#define PRIMS(X) \」で定義されて、用途に応じて 　　#define X1(op, name, code) op, 　　#define X2(op, name, code) NCASE op: code 　　#define X3(op, name, code) { op, name, 0 }, で利用されていると理解しています。 Cで記述されたCFのWORDはこのPRIMSで記述されたものですべてでしょうか？cf-boot.fthやdisk.xfは外部ファイルとして読み込まれて、PRIMS(X)に追加されていくのでしょうか？

理解はかなり合っています。ポイントは、

- **`PRIMS(X)` は「Cで実装された _プリミティブ(最下層ワード)_ の一覧表」**
...
**追加されません。**  
`PRIMS(X)` は **コンパイル時に固定**の「Cプリミティブ一覧」です。
...
- `disk.xf` = 名前から推測すると、さらに語彙を増やすための **“外部辞書イメージ/バイナリ形式”**または **追加スクリプト**のどちらか（ここはあなたの cf 実装の実物次第）
...
どちらでも **PRIMS に追加する**のではなく、**辞書領域を増やす/復元する**系です。
...
> 「CF起動時に _あらかじめ登録される FORTH の“変数・定数ワード”」です。__
...
`static struct {     char *nm;     cell  val; } nvp[] = {     { "base",   (cell)&base },     { "state",  (cell)&state },     { ">in",    (cell)&toIn },     { "here",   (cell)&here },     { "last",   (cell)&last },     { 0, 0 } };`
...
`base @      \ 現在の基数を読む 10 base !   \ 基数を10進にする`
...
`for (i = 0; nvp[i].nm; i++) {     addLit(nvp[i].nm, nvp[i].val); }`

- `nm == 0` になったら終了
    
- **配列サイズを別に持たなくて済む**ための C の定番イディオム
...
`┌──────────────┐ │ PRIMS(X)     │  ← Cプリミティブ（dup, swap, +, …） └──────────────┘         │ ┌──────────────┐ │ nvp[]        │  ← C変数をForthに公開（base, state, >in, here…） └──────────────┘         │ ┌──────────────┐ │ boot.fth     │  ← 高レベルForth語彙（: ; if do loop …） └──────────────┘         │ ┌──────────────┐ │ user scripts │  ← ユーザー定義 └──────────────┘`
...
この構造を理解できているのは、  
**Z80/MSX で CF を再構成するうえで「核心部分」を掴めている証拠**です。
...
では、この行を **CF の実行モデル（token-threaded VM）そのもの**として、噛み砕いて説明します。
...
`STOP,     // 0 LIT,      // 1 JMP,      // 2 JMPZ,     // 3 NJMPZ,    // 4 JMPNZ,    // 5 NJMPNZ,   // 6`
...
👉 **MSX/Z80 版を作るなら、最初は無視してもよい命令群**です  
（あとで最適化として入れる位置）
...
`enum {     STOP,     LIT,     JMP,     JMPZ,     NJMPZ,     JMPNZ,     NJMPNZ,      DUP,     SWAP,     DROP,     ADD,     SUB,     ... };`
...
`switch (op) {     case DUP:   ... break;     case ADD:   ... break; }`
...
色々教えいただいてありがとうございます。 まだ頭の中で整理中であります。今の私の理解では以下のようになります。正しいでしょうか？ 　・Cで記述されたCFのコアな部分では起動後に内容が追加されるのは定義されたスタック領域とワード変数だけである。 　・Cで記述されたCFのコアな部分は通常は変更されず、外部の.fthだけが状況に合わせて修正されていく。
...
`┌──────────────────────┐ │ Cで書かれたCFコア     │ ← 通常は固定 │                      │ │  - VM命令(enum/PRIMS)│ │  - データ/RSスタック │ │  - 辞書管理           │ │  - inner / outer     │ └─────────┬────────────┘           │           ▼ ┌──────────────────────┐ │ 起動時に増えるもの    │ │                      │ │  - nvp[]の変数ワード  │ │  - boot.fth の定義   │ │  - includeされた語彙 │ │                      │ │ → すべて辞書に追加   │ └─────────┬────────────┘           │           ▼ ┌──────────────────────┐ │ ユーザー定義 (.fth)   │ ← 状況に応じて変更 │                      │ │  - 新しいワード       │ │  - アプリ             │ │  - 実験               │ └──────────────────────┘`
...
`cfInit(); addLit(...); boot(...); while(1) repl();`
...
その「不思議な感覚」、まさに **Forth の起動モデルの核心**です。あなたが書いた FIG-FORTH の流れ（初期設定→ABORT→QUIT、でも QUIT の中が無限ループ）は、**“実体がないものから実体が現れる”というより「最小の実行機構が常駐していて、そこに言語が積み上がる」**という発想にぴったり合っています。
...
`main()  ├─ cfInit()           … VMの核を初期化（スタック/辞書/PRIMS登録）  ├─ addLit(arg...)     … 変数/定数ワードを追加（argc,arg0..）  ├─ boot(file?)        … 起動スクリプト読み込み（boot.fth等）  └─ while(1) repl()    … REPL/QUITループ（入力→解釈→実行→ok）`

### Z80/MSX-DOS版（startが“儀式”をやる）

`ORG 0100h JP start  start:  ├─ INIT相当            … SP/RS/変数/辞書/HERE/LAST/PRIMS準備  ├─ BOOT相当            … （任意）起動スクリプト相当を読む/内蔵する  └─ QUIT相当(loop):     … 無限ループ       ├─ KEY/ACCEPT      … 入力（1文字 or 1行）       ├─ INTERPRET       … トークン化→辞書探索→実行/コンパイル       ├─ ok表示          … プロンプト       └─ エラー時ABORT   … 状態を整えてQUITへ戻る`
...
`【INIT】（一度だけ）   - データスタック初期化  - リターンスタック初期化  - 変数初期化（BASE, STATE, >IN, HERE, LAST…）  - 辞書の最初の形を作る（PRIMS登録/ヘッダ構造準備）  - I/O初期化（BDOS/表示/キー入力）          ↓ 【ABORT】（いつでも呼ばれる“復帰儀式”）  - データスタックを空に  - STATEをインタプリタへ  - >INを0へ  - 必要ならエラーメッセージを表示  - （例外復帰点があればそこへ）          ↓ 【QUIT】（無限ループ）   loop:    - 入力を得る（ACCEPT/KEY）    - INTERPRET（単語→見つける→実行 or コンパイル）    - ok を出す    - goto loop`
...
`┌────────────────────────────────┐ │ [A] start / INIT                │  ← cfInit相当 │   - SP/RS設定                   │ │   - 変数領域初期化              │ │   - 辞書初期化(HERE/LAST)       │ │   - PRIMSテーブル登録           │ │   - 画面に起動メッセージ         │ └───────────────┬────────────────┘                 │ ┌───────────────▼────────────────┐ │ [B] ABORT（復帰ルーチン）        │ │   - DSを空に / STATE=0          │ │   - >IN=0 / エラーなら表示       │ │   - QUITへジャンプ              │ └───────────────┬────────────────┘                 │ ┌───────────────▼────────────────┐ │ [C] QUIT（外側ループ）           │ ← repl相当 │   loop:                         │ │     - ACCEPT/KEYで入力取得       │ │     - INTERPRET                 │ ← cfOuter相当 │     - ok表示                    │ │     - JP loop                   │ └───────────────┬────────────────┘                 │ ┌───────────────▼────────────────┐ │ [D] INTERPRET（外側インタプリタ）│ ← cfOuter │   - nextWord（単語切り出し）      │ │   - findWord（辞書探索）          │ │   - stateで分岐：                │ │      * 実行: executeWord         │ │      * compile: compileWord/LIT  │ │   - 数値なら LIT or push         │ └───────────────┬────────────────┘                 │ ┌───────────────▼────────────────┐ │ [E] INNER（内側インタプリタ）    │ ← cfInner │   - token列を実行（switch相当）  │ │   - PRIMS（dup/swap/drop等）     │ │   - STOPで復帰                  │ └────────────────────────────────┘`
...
いい質問です。あなたの FIG-FORTH の理解（**ABORT → QUIT** で“何事もなかったように入力待ちへ戻る”）は、CFでも **ほぼ同じ思想**で実現されています。ただし CF は FIG-FORTH のように「ABORT というワード名」が前面に出るというより、**C 側の構造（repl/cfOuter の戻り方）として ABORT/QUIT 相当を作っている**ことが多いです。
...
`main:   cfInit()   boot(...)   while(1) repl()    ← これが QUIT世界（外側ループ）`
...
CF の ABORT 相当は、普通は「1つの関数名」ではなく、**2層**に分かれて現れます。
...
のどちらかで、**深い入れ子（実行中）から一気に REPL へ戻る**構造を持つことがあります。
...
`INIT → ABORT → QUIT(無限)               └─ ACCEPT → INTERPRET → …`

### CF

`cfInit → boot → mainのwhile(1) { repl(); }                         └─ (ACCEPT) → cfOuter(tib) → …                               └─ エラーなら（ABORT相当の復帰処理）`
...
- 結果として、ユーザーは _次の入力待ち（ok）_ をまた見る
...
のように、**Cの制御構造として自然に見える**形になっていることが多いです。
...
を、FIG-FORTHの ABORT/QUIT と **1対1対応の図**で確定版として描けます。
