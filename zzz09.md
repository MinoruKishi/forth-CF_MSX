...
I/O：key（または行入力）, emit（または文字列出力）  
  
外側：tokenize、数値化、word lookup（ここは “outer” 相当）  
...
以後、ユーザー入力を cfOuter() が解釈して実行/コンパイルする  
...
PRIMS以外の“VM命令（LIT/JMP/BRANCH）”が別にある  
→ あなたの enum がそれを示しています。  
  
PRIMSのC部には “ホスト関数呼び出し” と “インタプリタ状態操作（toIn等）” が混ざっている  
...
ソフトウェアルーチンを primitive として持つ（おすすめ）  
mul16, div16, divmod16 を Z80 で用意して、* / /mod のワードはそれを呼ぶだけにする  
  
Forthで書く（可能だが“育つまで”が必要）  
...
inner interpreter（NEXT/dispatch）  
ワードを実行するための機械なので、ワードで書けない  
  
最小のスタック操作（push/pop と return stack）  
inner と分岐の基盤  
  
VM命令（少なくとも LIT と BRANCH 系）  
CFでは PRIMSの外側に LIT/JMP/JMPZ/... が存在するのが証拠  
  
ホスト呼び出しの橋（BDOS/BIOS callgate）  
...
Z80核：STOP/LIT/BRANCH/0BRANCH/EXIT + push/pop + NEXT  
  
最低I/Oワード：emit と key だけは callgate経由で primitive でもOK  
  
outer をWORD化する前に、CFの cfOuter 相当をまず Z80で動かして挙動確認  
  
動いたら outer の部品（tokenize/number/find/execute/compile）を順にWORD化  
...
最小スタック（push/pop + return stack）  
  
VM命令（LIT/BRANCH/EXITなど）  
...
NEXT/dispatch を自分で書く必要がある（Forthの卵問題）  
  
スタック規約を固定しないと、全部のprimitiveが崩れる  
  
LIT/BRANCH/EXIT 等のVM命令は PRIMS とは別枠として必須（CFでも別）  
  
BDOS/BIOS呼び出しの橋は必ず必要（WORD化のための土台）  
